  ...
  <div>
    <div class="con">
      <div class="lottery">
        ...
        <div class="lottery_r">
          <div class="tit">
            <img src="~@/assets/img/lottery_national_day/star.png" /> 中奖记录 <img src="~@/assets/img/lottery_national_day/star.png" />
          </div>
          <div class="record">
            <div class="record_c">
              <div class="list">
              ...
              <div class="gift_btn" @click="look">查看我的奖品</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <el-dialog :visible.sync="gift" center class="gift" :show-close="false" :close-on-click-modal="false">
      <div class="gift_c">
        ...
        <div class="information_c_none" v-if="lottery.status === '1'">
          ...
          <div class="btns" @click="gift = false">关闭</div>
        </div>
        <div class="information_c_block" v-else>
    ...
    <el-dialog :visible.sync="notwin" center class="notwin" :show-close="false" :close-on-click-modal="false">
      <i class="el-icon-circle-close ico" @click="notwin = false"></i>
      <div class="notwin_c">
        <p class="tit">您未中奖</p>
        <div class="peo">
          <img src="~@/assets/img/lottery/people.png" alt="" />
          <div class="txt">
            <p>很遗憾未中奖</p>
            <p>感谢您的参与</p>
          </div>
...
<script>
import { lottery, get, list, save } from "@/api/other/prizeWin.js";
...
export default {
  ...
  created() {
    this.listPrize();
    this.ruleForm.contactNumber = this.$store.getters["user/user"].phone;
  },
  methods: {
    // 获取奖品
    async listPrize() {
    ...
    start() {
      ...
      if (this.lottery && this.lottery.uuid) {
        this.$baseMessage("您已参与过抽奖，请您点击我的奖品进行查看");
        return;
      }
      ...
      lottery({ category: "国庆节抽奖活动", device: "PC" })
        .then((res) => {
          if (res.code === 200) {
            win = res.data;
          } else {
...
@import url("~@/assets/css/anon/lottery.less");
</style>
